...
const Sidebar = ({ isAuthenticated, loggedInUser, handleLogout }) => {
  ...
  return (
    <>
      <div className="fixed no-scrollbar top-0 left-0 h-screen w-64 bg-[#202c70] text-white shadow-lg z-10">
        <div className="flex items-center p-4 border-b border-white space-x-3">
        ...
        <div className={`mt-6 px-4 space-y-4 ${isMobileMenuOpen ? '' : 'hidden md:block'}`}>
          {isAuthenticated && (
            <>
              <Link to="/" className="block hover:text-yellow-400"><IoHome /> Home</Link>
              <Link to="/menu" className="block hover:text-yellow-400"><MdOutlineMenuBook /> Menu</Link>
              <Link to="/foods" className="block hover:text-yellow-400"><LiaFirstOrderAlt /> Order Now</Link>
              <Link to="/cart" className="block relative hover:text-yellow-400">
                <span className="absolute -top-2 left-14 bg-green-600 text-xs text-white rounded-full w-5 h-5 flex items-center justify-center">
                  0
                </span>
                <FaCartArrowDown /> Cart
              </Link>
              <Link to="#profile" className="block hover:text-yellow-400"><IoMdPerson /> Profile</Link>
              <div className="mt-4">
                <span className="block">Hi, {loggedInUser?.username}</span>
                <button onClick={handleLogout} className="hover:text-red-400 flex items-center"><AiOutlineLogout /> Logout</button>
              </div>
            </>
          )}
          ...
          {!isAuthenticated && (
            <>
              <Link to="/login" className="block hover:text-yellow-400"><IoMdLogIn /> Login</Link>
              <Link to="/register" className="block hover:text-yellow-400"><GiArchiveRegister /> Register</Link>
            </>
          )}
        </div>
        ...
        {/* Search Section */}
        {isAuthenticated && (
          <div className="mt-6 px-4 py-2 text-black bg-gray-100 rounded shadow">
            <input
              type="text"
              name="search"
              placeholder="Search foods..."
            ...
            <button
              onClick={handleSearch}
              className="bg-yellow-500 text-black px-3 py-1 rounded-md hover:bg-yellow-400 w-full"
            >
              Search
            </button>
          </div>
