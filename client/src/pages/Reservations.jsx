...
const MyReservations = () => {
  ...
  const [in_reservations, setInReservations] = useState([]);
  ...
  useEffect(() => {
    const fetchReservations = async () => {
      try {
        ...
        setInReservations(data.inactive_reservations);
        setLoading(false);
      } catch (err) {
  ...
  return (
    ...
    <div className="max-w-4xl mx-auto p-6">
      <h2 className="text-2xl font-semibold text-gray-800 mb-4">In Active Reservations</h2>

      {in_reservations.length === 0 ? (
        <p className="text-gray-600">You have no reservations.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {in_reservations.map((res) => (
            <div
              key={res.id}
              className="bg-white border-gray-200 rounded-lg shadow-sm p-4 hover:shadow-md transition-shadow duration-200"
            >
